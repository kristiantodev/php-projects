...
class Myprofil extends My_Controller {
public function __construct()
    ...
    {   
         ...
         $data["passwordku"] = $this->db->get_where('users', ['id_user' => $this->session->userdata('id_user')])->row_array();
         ...
         if($this->form_validation->run() == false){
          $this->page_form('isi/kaprodi/edit_password',$data);
         }else{
        ...
        if(!password_verify($lama, $data['passwordku']['password'])){
              ...
              redirect(site_url('page/kaprodi/myprofil/ganti_password'));
            }else{
               if($lama == $baru) {
